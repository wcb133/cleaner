//
//  ImageAndVideoAnalyseTool.swift
//  Cleaner
//
...
class ImageAndVideoAnalyseTool: NSObject {
    static let shared: ImageAndVideoAnalyseTool = {
        let instance = ImageAndVideoAnalyseTool()
        return instance
    }()
//图片
    //所有图片
    ...
    //相似照片
    var similarArray:[[ImageModel]] = []
    var similarSaveSpace = 0
    //截图
    var screenshotsArray:[ImageModel] = []
    var screenshotsSaveSpace = 0
    //可以瘦身图片
    var thinPhotoArray:[ImageModel] = []
    var thinPhotoSaveSpace:Int = 0
    //模糊图片
    var fuzzyPhotoArray:[ImageModel] = []
...
//        options.isNetworkAccessAllowed
    ...
    func dealImage(index:Int,exactImage:UIImage,originImageData:Data,analyseType:AnalyseType) {
        ...
        if let lastImageAsset = self.lastImageAsset {
            ...
            if isSameDay {
                let isLike = ImageCompareTool.isImage(self.lastThumImage, like: exactImage)
                if isLike {
        ...
        if asset.mediaSubtypes == .photoScreenshot {
            let model = ImageModel(asset: asset, exactImage: exactImage, originImageDataLength: originImageData.count)
...
//        模糊图片
        let isFuzzy = ImageCompareTool.isImageFuzzy(exactImage)
        if isFuzzy {
            let model = ImageModel(asset: asset, exactImage: exactImage, originImageDataLength: originImageData.count)
    ...
    func updateSimilarArr(asset:PHAsset,exactImage:UIImage,originImageData:Data) {
        if !self.isSameWithLastImage {//创建一组新的数据，因为是比较相似，把上一次的也添加进来
            let model = ImageModel(asset: self.lastImageAsset!, exactImage: self.lastThumImage!, originImageDataLength: self.lastOriImageData!.count)
...
extension ImageAndVideoAnalyseTool {
    ...
    func dealVideo(index:Int,exactImage:UIImage,videoAsset:AVAsset,analyseType:AnalyseType) {
        ...
        if let lastAsset = self.lastAsset {
            ...
            if isSameDay {
                let isLike = ImageCompareTool.isImage(self.lastImageOfVideo, like: exactImage)
                if isLike {
